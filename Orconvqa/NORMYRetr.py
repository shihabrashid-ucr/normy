...
searchDir = NIOFSDirectory(Paths.get('orqa_lucene_index/')) # PATH TO LUCENE INDEX
...
def retrieve_passage(qstn, searcher):
    ...
    topDocs = searcher.search(query, 10).scoreDocs # change k value here
...
def process_yake(passage): # for keyphrase extraction
...
def compare_encodings(pass1, pass2): # For comparing the similarity of two passages using SBERT
...
for d in range(0, len(dial)): #iterate every dialogue
        ...
        for h in range(0, len(historytext)): # appending all the history
            ...
            if querystr in passmap:
                ...
                if not qstn2:
                    qstn2 = 'null'
                    querystr = 'null'
                retrieved = retrieve_passage(qstn2, searcher) # returns k retrieved passages
            ...
            if h > 0: #update all previous scores
                for m in range(len(allpassages)-1 , -1, -1):
                    ...
                    if historyturn == h - 1: # returned passages from previous turns
                        sumssim = 0
                        ...
                        score *= avg_sim
                    allpassages[m] = (passage, score, historyturn, 0) # Update the item
            allpassages.extend(innerretlist)
    ...
    for t in range(0, len(allpassages)):
        ...
        top_10.append((allpassages[t][0], allpassages[t][1]))
        top10cntr += 1
    ...
    outermostlist.append(innerjson)
output_dir = 'history_models/orconvqa/'
if not os.path.exists(output_dir):
        os.makedirs(output_dir)
with open('history_models/orconvqa/normy_retr.json', 'w') as outfile:
