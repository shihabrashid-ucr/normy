...
with open('history_models/doc2dial/normy_retr.json') as f:
    dial = json.load(f)
...
for d in range(0, len(dial)):
    ...
    if history:
        ...
        for h in range(1, len(history)): # appending all the history
            if len(history) <= 6: # we will consider window = 6, change 2 here
    ...
    outermostlist.append(innerjson)
with open('history_models/doc2dial/normy_reranker.json', 'w') as outfile:
